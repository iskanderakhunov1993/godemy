...
export default function JuniorModulePage() {
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
      <div className="mb-8">
        <div className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-1 rounded-full bg-[#FFD60A]/10 text-[#FFD60A] border border-[#FFD60A]/20 mb-4">
          Bootcamp Junior
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <Link
          href="/junior/guide"
          className="card hover:bg-gray-800 transition-colors border-[#FFD60A]/20"
        >
          ...
          {sprints.map((s, i) => (
            <li key={s.label} className={`flex items-center gap-3 ${s.free ? 'text-gray-300' : 'text-gray-600'}`}>
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 ${
                s.free
                  ? 'bg-[#FFD60A]/20 border border-[#FFD60A]/30 text-[#FFD60A]'
