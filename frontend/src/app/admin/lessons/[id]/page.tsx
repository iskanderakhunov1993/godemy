...
function MarkdownPreview({ content }: { content: string }) {
  ...
  const html = content
    ...
    .replace(/```[\w]*\n([\s\S]*?)```/g, '<pre class="bg-gray-800 rounded-lg p-4 my-3 overflow-x-auto text-[#FFD60A] text-xs"><code>$1</code></pre>')
    .replace(/`([^`]+)`/g, '<code class="bg-gray-800 px-1.5 py-0.5 rounded text-[#FFD60A] text-xs">$1</code>')
    .replace(/\*\*(.+?)\*\*/g, '<strong class="text-white font-semibold">$1</strong>')
    .replace(/\*(.+?)\*/g, '<em class="text-gray-200">$1</em>')
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" class="rounded-lg max-w-full my-3" />')
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-[#FFD60A] underline hover:text-[#FFD60A]" target="_blank">$1</a>')
